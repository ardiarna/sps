...
if ($_REQUEST['mode'] == 'update' || $_REQUEST['mode'] == 'new') if (!has_privilege('trx.wo')) die ('access denied');
if ($_REQUEST['mode'] == 'delete') if (!has_privilege('trx.wo')) die ('access denied');

if ($_REQUEST['mode'] == 'export-all') {
    header("Content-Type: text/csv");
    header("Content-Disposition: attachment; filename=\"trx.wo-" . date("Y-m-d") . ".csv\"");
...
if (@mysql_query($cgx_sql, $cgx_connection)) {
    $_SESSION[$GLOBALS['APP_ID']]['trx.wo']['error'] = FALSE;
    $_SESSION[$GLOBALS['APP_ID']]['trx.wo']['info'] = 'Your data has been successfully updated';
    if ($_REQUEST['mode'] == 'new') $cgx_new_id = mysql_insert_id($cgx_connection);
} else {
    $_SESSION[$GLOBALS['APP_ID']]['trx.wo']['error'] = mysql_error($cgx_connection);
}
