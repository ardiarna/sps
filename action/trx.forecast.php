...
if ($_REQUEST['mode'] == 'update' || $_REQUEST['mode'] == 'new') if (!has_privilege('trx.so')) die ('access denied');
if ($_REQUEST['mode'] == 'delete') if (!has_privilege('trx.so')) die ('access denied');

if ($_REQUEST['mode'] == 'export-all') {
    header("Content-Type: text/csv");
    header("Content-Disposition: attachment; filename=\"trx.so-" . date("Y-m-d") . ".csv\"");
...
JOIN c_bpartner USING (c_bpartner_id)");
    ...
    $cgx_sql = "UPDATE c_forecast SET";
    $cgx_sql .= " document_no = '" . mysql_escape_string($_REQUEST['data']['document_no']) . "'";
    $cgx_sql .= " WHERE";
    $cgx_sql .= " c_forecast_id = '{$_REQUEST['pkey']['c_forecast_id']}'";
} elseif ($_REQUEST['mode'] == 'new') {
    $cgx_sql = "INSERT INTO c_forecast (";
    $cgx_sql .= "document_no";
    $cgx_sql .= ") values (";
    $cgx_sql .= "'" . mysql_escape_string($_REQUEST['data']['document_no']) . "'";
    $cgx_sql .= ")";
} elseif ($_REQUEST['mode'] == 'delete') {
    $cgx_sql = "DELETE FROM c_forecast ";
    $cgx_sql .= " WHERE";
    $cgx_sql .= " c_forecast_id = '{$_REQUEST['pkey']['c_forecast_id']}'";
    
    mysql_query("DELETE FROM c_forecast_line WHERE c_forecast_id = '{$_REQUEST['pkey']['c_forecast_id']}'", $APP_CONNECTION);
}

if (@mysql_query($cgx_sql, $cgx_connection)) {
    $_SESSION[$GLOBALS['APP_ID']]['trx.so']['error'] = FALSE;
    $_SESSION[$GLOBALS['APP_ID']]['trx.so']['info'] = 'Your data has been successfully updated';
    if ($_REQUEST['mode'] == 'new') $cgx_new_id = mysql_insert_id($cgx_connection);
} else {
    $_SESSION[$GLOBALS['APP_ID']]['trx.so']['error'] = mysql_error($cgx_connection);
}
