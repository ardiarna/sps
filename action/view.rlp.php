...
LEFT JOIN app_user auu ON (m_work_order.update_user=auu.user_id) WHERE m_work_order.type_id = 'R' AND " . org_filter_trx('m_work_order.app_org_id');
...
if ($_REQUEST['mode'] == 'export-all') {
    ...
    header("Content-Disposition: attachment; filename=\"request-long-pipe-" . date("Y-m-d") . ".csv\"");
    echo "\"NO. REQUEST\"";
    ...
    while (($cgx_dt_export = mysql_fetch_array($cgx_rs_export, MYSQL_ASSOC)) !== FALSE) {
        ...
        echo ",\"" . str_replace("\"", "\"\"", $cgx_dt_export['material_quantity']) . "\"";
...
if (@mysql_query($cgx_sql, $cgx_connection)) {
    $_SESSION[$GLOBALS['APP_ID']]['view.rlp']['error'] = FALSE;
    $_SESSION[$GLOBALS['APP_ID']]['view.rlp']['info'] = 'Data anda sudah berhasil diperbarui';
    if ($_REQUEST['mode'] == 'new') $cgx_new_id = $_REQUEST['data']['Array'];
} else {
    $_SESSION[$GLOBALS['APP_ID']]['view.rlp']['error'] = mysql_error($cgx_connection);
}
