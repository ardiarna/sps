...
$cgx_sql = "SELECT mmr.*, mp1.product_code, mp1.spec, mp1.od, mp1.thickness, mp1.length, partner_name,
            ...
            JOIN m_product mp2 ON(mmr.m_product_material=mp2.m_product_id) WHERE mp1.category = 'L' AND mp2.category = 'L' ";
...
if ($_REQUEST['mode'] == 'update' || $_REQUEST['mode'] == 'new') if (!has_privilege('master.materi_lp')) die ('akses ditolak');
if ($_REQUEST['mode'] == 'delete') if (!has_privilege('master.materi_lp')) die ('akses ditolak');

if ($_REQUEST['mode'] == 'export-all') {
    header("Content-Type: text/csv");
    header("Content-Disposition: attachment; filename=\"master.materi_lp-" . date("Y-m-d") . ".csv\"");
    echo "\"MATERIAL ID\"";
    echo ",\"ITEM NUMBER OUT\"";
    echo ",\"CUSTOMER\"";
    echo ",\"SPEC\"";
    echo ",\"OD\"";
    echo ",\"THICKNESS\"";
...
if (@mysql_query($cgx_sql, $cgx_connection)) {
    $_SESSION[$GLOBALS['APP_ID']]['master.materi_lp']['error'] = FALSE;
    $_SESSION[$GLOBALS['APP_ID']]['master.materi_lp']['info'] = 'Data anda sudah berhasil diperbarui';
    if ($_REQUEST['mode'] == 'new') $cgx_new_id = mysql_insert_id($cgx_connection);
}else {
    $_SESSION[$GLOBALS['APP_ID']]['master.materi_lp']['error'] = mysql_error($cgx_connection);
}
