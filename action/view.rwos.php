...
$cgx_sql = "SELECT
            ...
            mp2.length, auc.user_fullname, auu.user_fullname user_fullname_u, m_prod_slit_line.good as good, 
            m_prod_slit_line.weight as weight, m_warehouse.warehouse_name 
...
if ($_REQUEST['mode'] == 'export-all') {
    ...
    header("Content-Disposition: attachment; filename=\"realisasi-work-order-pipa-" . date("Y-m-d") . ".csv\"");
    echo "\"NO. PRODUKSI\"";
...
if (@mysql_query($cgx_sql, $cgx_connection)) {
    $_SESSION[$GLOBALS['APP_ID']]['view.rwos']['error'] = FALSE;
    $_SESSION[$GLOBALS['APP_ID']]['view.rwos']['info'] = 'Data anda sudah berhasil diperbarui';
    if ($_REQUEST['mode'] == 'new') $cgx_new_id = $_REQUEST['data']['Array'];
} else {
    $_SESSION[$GLOBALS['APP_ID']]['view.rwos']['error'] = mysql_error($cgx_connection);
}
