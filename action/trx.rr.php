...
if ($_REQUEST['mode'] == 'delete') if (!has_privilege('trx.barang-masuk')) die ('access denied');
...
if (@mysql_query($cgx_sql, $cgx_connection)) {
    $_SESSION[$GLOBALS['APP_ID']]['trx.rr']['error'] = FALSE;
    $_SESSION[$GLOBALS['APP_ID']]['trx.rr']['info'] = 'Your data has been successfully updated';
    if ($_REQUEST['mode'] == 'new') $cgx_new_id = mysql_insert_id($cgx_connection);
} else {
    $_SESSION[$GLOBALS['APP_ID']]['trx.rr']['error'] = mysql_error($cgx_connection);
}
