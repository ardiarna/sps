...
if ($_REQUEST['mode'] == 'export-all') {
    ...
    echo "\"Nomor Dokumen\",\"Tgl Penerimaan\",\"Kode Customer\",\"Nama Customer\",\"Item Number\",\"Spec\",\"OD\",\"Thickness\",\"Length\",\"Description\",\"Gudang\",\"Jumlah Barang\"\n";
...
if (@mysql_query($cgx_sql, $cgx_connection)) {
    $_SESSION[$GLOBALS['APP_ID']]['view.rr']['error'] = FALSE;
    $_SESSION[$GLOBALS['APP_ID']]['view.rr']['info'] = 'Data anda sudah berhasil diperbarui';
    if ($_REQUEST['mode'] == 'new') $cgx_new_id = $_REQUEST['data']['Array'];
} else {
    $_SESSION[$GLOBALS['APP_ID']]['view.rr']['error'] = mysql_error($cgx_connection);
}
