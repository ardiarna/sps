...
if ($_REQUEST['mode'] == 'update' || $_REQUEST['mode'] == 'new') if (!has_privilege('master.product')) die ('akses ditolak');
if ($_REQUEST['mode'] == 'delete') if (!has_privilege('master.product')) die ('akses ditolak');

if ($_REQUEST['mode'] == 'export-all') {
    header("Content-Type: text/csv");
    header("Content-Disposition: attachment; filename=\"master.product-" . date("Y-m-d") . ".csv\"");
    echo "\"ID\",\"Organization\",\"Kode Produk\",\"Spec\",\"OD\",\"Thickness\",\"Length\",\"Nama Produk\",\"Description\",\"Minimum Quantity\",\"Purchase\",\"Sale\",\"Kategori\",\"Active\"\n";
    $cgx_rs_export = mysql_query("SELECT m_product.*, organization FROM m_product JOIN app_org USING (app_org_id) WHERE category = 'L' OR category = 'C'", $cgx_connection);
    ...
    $cgx_sql .= ", model = '" . mysql_escape_string($_REQUEST['data']['model']) . "'";
    ...
    $cgx_sql .= "app_org_id,c_bpartner_id,product_code,spec,thickness,od,length,product_name,description,model,minimum_qty,purchase,sale,category,active";
    $cgx_sql .= ") values (";
    ...
    $cgx_sql .= ",'" . mysql_escape_string($_REQUEST['data']['model']) . "'";
...
if (@mysql_query($cgx_sql, $cgx_connection)) {
    $_SESSION[$GLOBALS['APP_ID']]['master.product']['error'] = FALSE;
    $_SESSION[$GLOBALS['APP_ID']]['master.product']['info'] = 'Data anda sudah berhasil diperbarui';
    if ($_REQUEST['mode'] == 'new') $cgx_new_id = mysql_insert_id($cgx_connection);
} else {
    $_SESSION[$GLOBALS['APP_ID']]['master.product']['error'] = mysql_error($cgx_connection);
}
