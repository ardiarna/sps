...
echo "<div class='title'>Hasil Produksi </div>";
...
if (strlen($_REQUEST['pkey']['m_inout_id']) > 0) {
    include_once 'trx.hp.edit.php';
} else {
    require_once 'Structures/DataGrid.php';
    require_once 'HTML/Table.php';

    if (is_array($_SESSION[$GLOBALS['APP_ID']]['trx.hp']['columns'])) {
        $cgx_def_columns = $_SESSION[$GLOBALS['APP_ID']]['trx.hp']['columns'];
    } else {
        ...
        $_SESSION[$GLOBALS['APP_ID']]['trx.hp']['columns'] = $cgx_def_columns;
    }

    $cgx_sql = "SELECT m_inout_id, m_inout_date, m_inout.document_no, c_spk.document_no as spk FROM m_inout LEFT JOIN c_spk USING (c_spk_id) WHERE m_inout.m_transaction_type_id = 10 ";
    ...
    if ($cgx_def_columns['spk'] == 1) $cgx_datagrid->addColumn(new Structures_DataGrid_Column('Nomor SPK', 'spk', 'spk', array('align' => 'left'), NULL, NULL));
    if (has_privilege('trx.hp')) $cgx_datagrid->addColumn(new Structures_DataGrid_Column(NULL, NULL, NULL, array('align' => 'center', 'width' => '1'), NULL, 'cgx_edit()'));
//    if (has_privilege('trx.hp')) $cgx_datagrid->addColumn(new Structures_DataGrid_Column(NULL, NULL, NULL, array('align' => 'center', 'width' => '1'), NULL, 'cgx_delete()'));
