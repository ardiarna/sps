...
function cgx_delete($data) {
    if ($data['record']['status'] == 'C') return;
    $href  = "javascript:if(confirm('Hapus baris ini?')){window.location='action/trx.puo.php";
    $href .= "?backvar=module.php%253F%2526m%253Dtrx.puo&mode=delete&pkey[c_order_id]={$data['record']['c_order_id']}';}";
    $out = "<a href=\"{$href}\"><img title='Delete this row' src='images/icon_delete.png' border='0'></a>";
    return $out;
}

function grid_puo($data) {
    $href = "module.php?m={$_REQUEST['m']}&pkey[c_order_id]={$data['record']['c_order_id']}";
...
if (strlen($_REQUEST['pkey']['c_order_id']) > 0) {
    include_once 'trx.puo.edit.php';
} else {
    require_once 'Structures/DataGrid.php';
    require_once 'HTML/Table.php';

    if (is_array($_SESSION[$GLOBALS['APP_ID']]['trx.puo']['columns'])) {
        $cgx_def_columns = $_SESSION[$GLOBALS['APP_ID']]['trx.puo']['columns'];
    } else {
        ...
        $_SESSION[$GLOBALS['APP_ID']]['trx.puo']['columns'] = $cgx_def_columns;
    }
    ...
    $cgx_sql = "SELECT c_order_id, document_no, partner_name, order_date, reference_no, remark, status, c_order.app_org_id as org_id, organization
                ...
                JOIN app_org ON(c_order.app_org_id=app_org.app_org_id) WHERE m_transaction_type_id = 2 ";
    ...
    echo "<td>" . cgx_filter('status', $arr_status, $_REQUEST['status'], FALSE, "onclick=\"window.document = 'module.php?m=trx.puo&q={$_REQUEST['q']}&status={$_REQUEST['status']}';\"") . "</td>";
    ...
    echo "<input type='hidden' name='dg_name' value='trx.puo'>\n";
    ...
    echo "<table cellspacing='0' cellpadding='0' class='datagrid_bar_customize' align='left' width='160'><tr><td width='1'><input" . ($cgx_def_columns['remark'] == 1 ? ' checked' : '') . " id='col_remark' name='col[remark]' type='checkbox'></td><td width='99%'><label for='col_remark'>Contract Number</label></td></tr></table>\n";
...
<?php
    ...
    if ($_SESSION[$GLOBALS['APP_ID']]['trx.puo']['error']) {
        echo "<div class='error'>{$_SESSION[$GLOBALS['APP_ID']]['trx.puo']['error']}</div>";
        unset($_SESSION[$GLOBALS['APP_ID']]['trx.puo']['error']);
    }

    if ($_SESSION[$GLOBALS['APP_ID']]['trx.puo']['info']) {
        echo "<div class='info'>{$_SESSION[$GLOBALS['APP_ID']]['trx.puo']['info']}</div>";
        unset($_SESSION[$GLOBALS['APP_ID']]['trx.puo']['info']);
    }
    ...
    if ($cgx_def_columns['document_no'] == 1) $cgx_datagrid->addColumn(new Structures_DataGrid_Column('Nomor Dokumen', 'document_no', 'document_no', array('align' => 'left'), NULL, "grid_puo"));
