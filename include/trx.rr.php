...
echo "<div class='title'>".title_header('trx.rr', org())."</div>";
...
function cgx_delete($data) {
    $href  = "javascript:if(confirm('Hapus baris ini?')){window.location='action/trx.rr.php";
    $href .= "?backvar=module.php%253F%2526m%253Dtrx.rr&mode=delete&pkey[m_inout_id]={$data['record']['m_inout_id']}';}";
    $out = "<a href=\"{$href}\"><img title='Delete this row' src='images/icon_delete.png' border='0'></a>";
    return $out;
}

if (strlen($_REQUEST['pkey']['m_inout_id']) > 0) {
    include_once 'trx.rr.edit.php';
} else {
    require_once 'Structures/DataGrid.php';
    require_once 'HTML/Table.php';

    if (is_array($_SESSION[$GLOBALS['APP_ID']]['trx.rr']['columns'])) {
        $cgx_def_columns = $_SESSION[$GLOBALS['APP_ID']]['trx.rr']['columns'];
    } else {
        ...
        $_SESSION[$GLOBALS['APP_ID']]['trx.rr']['columns'] = $cgx_def_columns;
    }

    $cgx_sql = "SELECT * FROM m_inout join c_bpartner on m_inout.tuj_org_id=c_bpartner.c_bpartner_id WHERE m_transaction_type_id = 3";
    ...
    echo "<td></td>\n";
    echo "<td width='20'></td>\n";
    if (has_privilege('trx.barang-masuk')) {
    ...
    echo "</tr></table>\n";
    echo "</form>\n";
    $cgx_sql .= " and ( m_inout.m_inout_id LIKE '%{$cgx_search}%' OR m_inout.document_no LIKE '%{$cgx_search}%' OR partner_name LIKE '%{$cgx_search}%' OR m_inout.dokumen LIKE '%{$cgx_search}%')";
    ...
    if (has_privilege('trx.barang-masuk')) $cgx_datagrid->addColumn(new Structures_DataGrid_Column(NULL, NULL, NULL, array('align' => 'center', 'width' => '1'), NULL, 'cgx_edit()'));
    //if (has_privilege('trx.barang-masuk')) $cgx_datagrid->addColumn(new Structures_DataGrid_Column(NULL, NULL, NULL, array('align' => 'center', 'width' => '1'), NULL, 'cgx_delete()'));
