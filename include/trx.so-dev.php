...
echo "<div class='title'>Delivery Sales Order</div>";
...
function cgx_delete($data) {
    if ($data['record']['status'] == 'C') return;
    $href  = "javascript:if(confirm('Hapus baris ini?')){window.location='action/trx.so-dev.php";
    $href .= "?backvar=module.php%253F%2526m%253Dtrx.so-dev&mode=delete&pkey[c_order_id]={$data['record']['c_order_id']}';}";
...
if (strlen($_REQUEST['pkey']['c_order_id']) > 0) {
    include_once 'trx.so-dev.edit.php';
} else {
    require_once 'Structures/DataGrid.php';
    require_once 'HTML/Table.php';

    if (is_array($_SESSION[$GLOBALS['APP_ID']]['trx.so-dev']['columns'])) {
        $cgx_def_columns = $_SESSION[$GLOBALS['APP_ID']]['trx.so-dev']['columns'];
    } else {
        ...
        $_SESSION[$GLOBALS['APP_ID']]['trx.so-dev']['columns'] = $cgx_def_columns;
    }
...
JOIN c_bpartner USING (c_bpartner_id) WHERE 1 = 1";
    ...
    echo "<td>" . cgx_filter('status', $arr_status, $_REQUEST['status'], FALSE, "onclick=\"window.document = 'module.php?m=trx.so-dev&q={$_REQUEST['q']}&status={$_REQUEST['status']}';\"") . "</td>";
    ...
    echo "<input type='hidden' name='dg_name' value='trx.so-dev'>\n";
...
<?php
    ...
    if ($_SESSION[$GLOBALS['APP_ID']]['trx.so-dev']['error']) {
        echo "<div class='error'>{$_SESSION[$GLOBALS['APP_ID']]['trx.so-dev']['error']}</div>";
        unset($_SESSION[$GLOBALS['APP_ID']]['trx.so-dev']['error']);
    }

    if ($_SESSION[$GLOBALS['APP_ID']]['trx.so-dev']['info']) {
        echo "<div class='info'>{$_SESSION[$GLOBALS['APP_ID']]['trx.so-dev']['info']}</div>";
        unset($_SESSION[$GLOBALS['APP_ID']]['trx.so-dev']['info']);
    }
