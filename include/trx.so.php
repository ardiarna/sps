...
function cgx_delete($data) {
    if ($data['record']['status'] == 'C') return;
    $href  = "javascript:if(confirm('Hapus baris ini?')){window.location='action/trx.so.php";
    $href .= "?backvar=module.php%253F%2526m%253Dtrx.so&mode=delete&pkey[c_order_id]={$data['record']['c_order_id']}';}";
...
if (strlen($_REQUEST['pkey']['c_order_id']) > 0) {
    include_once 'trx.so.edit.php';
} else {
    require_once 'Structures/DataGrid.php';
    require_once 'HTML/Table.php';

    if (is_array($_SESSION[$GLOBALS['APP_ID']]['trx.so']['columns'])) {
        $cgx_def_columns = $_SESSION[$GLOBALS['APP_ID']]['trx.so']['columns'];
    } else {
        ...
        $_SESSION[$GLOBALS['APP_ID']]['trx.so']['columns'] = $cgx_def_columns;
    }
    ...
    $cgx_sql = "SELECT c_order_id, document_no, partner_name, order_date, reference_no, remark, status, c_order.app_org_id as org_id, organization
                ...
                JOIN app_org ON(c_order.app_org_id=app_org.app_org_id) WHERE m_transaction_type_id = 1 ";
    ...
    echo "<input type='hidden' name='dg_name' value='trx.so'>\n";
