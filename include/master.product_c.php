...
echo "<div class='title'>Master Data Produk Coil</div>";

function cgx_format_category($data) {
    $arr = array('R' => 'Raw Material', 'S' => 'Slitting');
...
function cgx_delete($data) {
    $href  = "javascript:if(confirm('Menghapus baris ini?')){window.location='action/master.product_c.php";
    $href .= "?backvar=module.php%253F%2526m%253Dmaster.product_c&mode=delete&pkey[m_product_id]={$data['record']['m_product_id']}';}";
...
if (strlen($_REQUEST['pkey']['m_product_id']) > 0) {
    ...
    echo "<form action='action/master.product_c.php' method='post'>\n";
    ...
    if ($_SESSION[$GLOBALS['APP_ID']]['master.product_c']['error']) {
        echo "<div class='error'>{$_SESSION[$GLOBALS['APP_ID']]['master.product_c']['error']}</div>";
        unset($_SESSION[$GLOBALS['APP_ID']]['master.product_c']['error']);
    }

    if ($_SESSION[$GLOBALS['APP_ID']]['master.product_c']['info']) {
        echo "<div class='info'>{$_SESSION[$GLOBALS['APP_ID']]['master.product_c']['info']}</div>";
        unset($_SESSION[$GLOBALS['APP_ID']]['master.product_c']['info']);
    }
    ...
    //========================================================================================
    echo "    <li class='even'>\n";
    echo "        <label class='cgx_form' for='partner_name'>Vendor</label>\n";
    echo "        <input id='partner_name' type='text' value=\"{$cgx_data['partner_name']}\" size='30' maxlength='40' style='text-align: left;' /><img onclick=\"popupReference('business-partner-v');\" style='cursor: pointer; vertical-align: bottom;' src='images/icon_reference.png'>\n";
    echo "    </li>\n";
    //========================================================================================
    ...
    echo "        <input id='data_spec' name='data[spec]' type='text' value=\"{$cgx_data['spec']}\" size='20' maxlength='20' style='text-align: left;' />\n";
    echo "    </li>\n";
    //========================================================================================
    ...
    echo "    <li class='odd'>\n";
    echo "        <label class='cgx_form' for='data_length'>OD</label>\n";
    ...
    //============================================================================================
    echo "<li class='even'>\n";
    echo "        <label class='cgx_form' for='data_category'>Category</label>\n";
    echo cgx_form_select('data[category]', array('R' => 'Raw Material', 'S' => 'Slitting'), $cgx_data['category'], FALSE, "id='data_category'");
    echo "    </li>\n";
    //=============================================================================================
    echo "    <li class='odd'>\n";
    ...
    echo "        <input type='button' value='Kembali' onclick=\"window.location = 'module.php?&m=master.product_c';\">\n";
    echo "    </li>\n";
    echo "</ul>\n";
    echo "</form>\n";
...
<?php
    ...
    if (is_array($_SESSION[$GLOBALS['APP_ID']]['master.product_c']['columns'])) {
        $cgx_def_columns = $_SESSION[$GLOBALS['APP_ID']]['master.product_c']['columns'];
    } else {
        ...
        $_SESSION[$GLOBALS['APP_ID']]['master.product_c']['columns'] = $cgx_def_columns;
    }
    ...
    echo "<td>" . cgx_filter('f4', array('R' => 'Raw Material', 'S' => 'Slitting'), $cgx_filter4, TRUE) . "</td>\n";
    ...
    echo "<td></td>\n";
    echo "<td width='20'></td>\n";
    if (has_privilege('master.product_c')) {
    ...
    echo "<input type='hidden' name='dg_name' value='master.product_c'>\n";
...
<?php
    ...
    if (strlen($cgx_filter4) > 0) {
        $cgx_sql .= " AND category = '" . mysql_escape_string($cgx_filter4) . "'";
    }else{
        $cgx_sql .= " AND (category = 'R' OR category = 'S')";
    }
    $cgx_sql .= " and ( m_product.m_product_id LIKE '%{$cgx_search}%' OR m_product.product_code LIKE '%{$cgx_search}%' OR m_product.product_name LIKE '%{$cgx_search}%')";
    ...
    if ($cgx_def_columns['category'] == 1) $cgx_datagrid->addColumn(new Structures_DataGrid_Column('Categori', 'category', 'category', array('align' => 'center'), NULL, "cgx_format_category()"));
