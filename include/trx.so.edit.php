...
if ($_REQUEST['mode'] == 'edit' && $_SESSION[$APP_ID]['order']['c_order_id'] == $_REQUEST['pkey']['c_order_id']) {
    $data = $_SESSION[$APP_ID]['order'];
} else {
...
if ($_SESSION[$GLOBALS['APP_ID']]['trx.so']['error']) {
    echo "<div class='error'>{$_SESSION[$GLOBALS['APP_ID']]['trx.so']['error']}</div>";
    unset($_SESSION[$GLOBALS['APP_ID']]['trx.so']['error']);
}

if ($_SESSION[$GLOBALS['APP_ID']]['trx.so']['info']) {
    echo "<div class='info'>{$_SESSION[$GLOBALS['APP_ID']]['trx.so']['info']}</div>";
    unset($_SESSION[$GLOBALS['APP_ID']]['trx.so']['info']);
}
...
if ($_REQUEST['mode'] == 'edit') {
    ...
    echo "<input" . ($data['status'] == 'C' ? ' disabled' : '')  . " type='button' value='Cetak' onclick=\"window.location = 'report.php?path=/reports/SPS/SO&param[REPORT_ID_PENGIRIMAN]={$data['c_order_id']}&type=docx&fname={$data['document_no']}'\">";
