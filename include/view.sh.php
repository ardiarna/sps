...
// Pengiriman Barang berdasarkan sales order

echo "<div class='title'>".title_header('view.sh', org())."</div>";

function grid_so($data) {
    $href = "module.php?m=trx.so&back_to=view.sh&mode=view&pkey[c_order_id]={$data['record']['c_order_id']}";
    $out = "<a href='{$href}'>{$data['record']['so_number']}</a>";
    return $out;
}
...
if (is_array($_SESSION[$GLOBALS['APP_ID']]['view.sh']['columns'])) {
    $cgx_def_columns = $_SESSION[$GLOBALS['APP_ID']]['view.sh']['columns'];
} else {
    ...
    $_SESSION[$GLOBALS['APP_ID']]['view.sh']['columns'] = $cgx_def_columns;
}

$cgx_sql = "SELECT m_inout.document_no, m_inout.m_inout_date, m_inout.dokumen, m_inout.no_kendaraan, m_inout.c_order_id,  
c_order_line.schedule_delivery_date, c_order.remark so_number, c_order.order_date,
partner_code, partner_name, product_code, product_name, od, thickness, length,
warehouse_name, m_inout_line.*, m_inout.create_date createdate, m_inout.update_date, auc.user_fullname, auu.user_fullname user_fullname_u
FROM m_inout
...
echo "<input type='hidden' name='dg_name' value='view.sh'>\n";
...
echo "<table cellspacing='0' cellpadding='0' class='datagrid_bar_customize' align='left' width='160'><tr><td width='1'><input" . ($cgx_def_columns['no_kendaraan'] == 1 ? ' checked' : '') . " id='col_no_kendaraan' name='col[no_kendaraan]' type='checkbox'></td><td width='99%'><label for='col_no_kendaraan'>No. Kendaraan</label></td></tr></table>\n";
echo "<table cellspacing='0' cellpadding='0' class='datagrid_bar_customize' align='left' width='160'><tr><td width='1'><input disabled checked id='col_schedule_delivery_date' name='col[schedule_delivery_date]' type='checkbox'></td><td width='99%'><label for='col_schedule_delivery_date'>Jadwal Kirim</label></td></tr></table>\n";
...
echo "<table cellspacing='0' cellpadding='0' class='datagrid_bar_customize' align='left' width='160'><tr><td width='1'><input disabled checked id='col_so_number' name='col[so_number]' type='checkbox'></td><td width='99%'><label for='col_so_number'>Remark</label></td></tr></table>\n";
...
function exportCSV() {
    ...
    form.setAttribute("action", "action/view.sh.php");
    ...
    hiddenField.setAttribute("value", "<?php echo title_header('view.sh', org()); ?>");
    form.appendChild(hiddenField);
...
if ($cgx_def_columns['so_number'] == 1) $cgx_datagrid->addColumn(new Structures_DataGrid_Column('Remark', 'so_number', 'so_number', array('align' => 'left'), NULL, "grid_so"));
