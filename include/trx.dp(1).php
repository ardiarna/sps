...
echo "</tr>";
$rsx = mysql_query(
        "SELECT c_delivery_plan.m_product_id, product_code, spec, od, thickness, length, mid(partner_name,1,20) partner_name, c_order.document_no, c_order.remark, 
        ...
        GROUP BY m_product_id, product_code ORDER BY partner_name, m_product_id",
        $APP_CONNECTION);
while ($dtx = mysql_fetch_array($rsx, MYSQL_ASSOC)) {
    ...
    $rsz = mysql_query(
            "SELECT DISTINCT c_order.document_no sales_order, remark
            FROM c_delivery_plan
            JOIN c_order_line ON (c_delivery_plan.order_line_ref = c_order_line.c_order_line_id)
            JOIN c_order USING (c_order_id)
            WHERE c_delivery_plan.schedule_delivery_date BETWEEN '{$d1sql}' AND '{$d2sql}'
    ...
    for ($t = $ts1; $t <= $ts2; $t += 86400) {
        ...
        if ($data[$d]['order_quantity'] > 0) {
            if ($_REQUEST['ref'] == $data[$d]['order_line_ref']) {
                ...
                $link = "module.php?m=trx.dp&d1={$d1}&d2={$d2}&org={$def_org}&product={$data[$d]['m_product_id']}&ref={$data[$d]['order_line_ref']}";
