...
echo "<div class='title'>Create SPK Long Pipe</div>";
...
echo "<input type='hidden' name='m' value='trx.ppc-sp-lp'>";
...
if ($_REQUEST['machine'] && $_REQUEST['wo']) {
    echo "<form id='frmSPKexec' action='action/trx.ppc-sp-lp.php' method='POST'>";
    echo "<input type='hidden' id='wo' name='wo' value='{$_REQUEST['wo']}'>";
    echo "<input type='hidden' id='d' name='d' value='{$_REQUEST['d']}'>";
    ...
    $rsx = mysql_query(
            "SELECT c_wo_line.*, product_code, balance_quantity FROM c_wo_line  "
            . "JOIN m_product USING (m_product_id) "
            . "LEFT JOIN (SELECT m_product_id, balance_quantity, max(balance_date) FROM m_stock_balance_d_2 WHERE balance_date <= '{$_REQUEST['d']}' GROUP BY m_product_id) sb USING (m_product_id) "
