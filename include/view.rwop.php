...
if (is_array($_SESSION[$GLOBALS['APP_ID']]['view.rwop']['columns'])) {
    $cgx_def_columns = $_SESSION[$GLOBALS['APP_ID']]['view.rwop']['columns'];
} else {
    ...
    $_SESSION[$GLOBALS['APP_ID']]['view.rwop']['columns'] = $cgx_def_columns;
}

$cgx_sql = "SELECT m_prod_slit.*, m_wo_pipa.document_no wo, m_wo_pipa_line.*, 
            ...
            m_prod_slit_line.good as good,
            m_prod_slit_line.weight as weight,
...
echo "      <td align='right'>Spec - OD x T x L</td>\n";
echo "      <td colspan='4' align='left'>" . cgx_filter('spec', "SELECT DISTINCT spec, spec FROM m_prod_slit_line JOIN m_wo_pipa_line ON (m_prod_slit_line.m_wo_slit_line_id = m_wo_pipa_line.m_wo_pipa_line_id) JOIN m_product ON(m_wo_pipa_line.m_product_id=m_product.m_product_id) ORDER BY spec", $spec, TRUE)." - ". cgx_filter('od', "SELECT DISTINCT od, od FROM m_prod_slit_line JOIN m_wo_pipa_line ON (m_prod_slit_line.m_wo_slit_line_id = m_wo_pipa_line.m_wo_pipa_line_id) JOIN m_product ON(m_wo_pipa_line.m_product_id=m_product.m_product_id) ORDER BY od", $od, TRUE) ." x ". cgx_filter('thickness', "SELECT DISTINCT thickness, thickness FROM m_prod_slit_line JOIN m_wo_pipa_line ON (m_prod_slit_line.m_wo_slit_line_id = m_wo_pipa_line.m_wo_pipa_line_id) JOIN m_product ON(m_wo_pipa_line.m_product_id=m_product.m_product_id) ORDER BY thickness", $thickness, TRUE) ." x ". cgx_filter('length', "SELECT DISTINCT length, length FROM m_prod_slit_line JOIN m_wo_pipa_line ON (m_prod_slit_line.m_wo_slit_line_id = m_wo_pipa_line.m_wo_pipa_line_id) JOIN m_product ON(m_wo_pipa_line.m_product_id=m_product.m_product_id) ORDER BY length", $length, TRUE) ."</td>\n";
echo "  </tr></table>\n";
echo "</form>\n";
...
echo "<input type='hidden' name='dg_name' value='view.rwop'>\n";
...
echo "<table cellspacing='0' cellpadding='0' class='datagrid_bar_customize' align='left' width='160'><tr><td width='1'><input" . ($cgx_def_columns['good'] == 1 ? ' checked' : '') . " id='col_good' name='col[good]' type='checkbox'></td><td width='99%'><label for='col_good'>Qty Good</label></td></tr></table>\n";
echo "<table cellspacing='0' cellpadding='0' class='datagrid_bar_customize' align='left' width='160'><tr><td width='1'><input" . ($cgx_def_columns['weight'] == 1 ? ' checked' : '') . " id='col_weight' name='col[weight]' type='checkbox'></td><td width='99%'><label for='col_weight'>Qty Weight</label></td></tr></table>\n";
...
function exportCSV() {
    ...
    form.setAttribute("action", "action/view.rwop.php");
...
if ($wo) $cgx_sql .= " AND m_wo_pipa.document_no LIKE '%" . mysql_escape_string($wo) . "%'";
if (strlen($spec) > 0) {
    $cgx_sql .= " AND mp2.spec = '" . mysql_escape_string($spec) . "'";
}
if (strlen($od) > 0) {
    $cgx_sql .= " AND mp2.od = '" . mysql_escape_string($od) . "'";
}
if (strlen($thickness) > 0) {
    $cgx_sql .= " AND mp2.thickness = '" . mysql_escape_string($thickness) . "'";
}
if (strlen($length) > 0) {
    $cgx_sql .= " AND mp2.length = '" . mysql_escape_string($length) . "'";
}
...
if ($cgx_def_columns['good'] == 1) $cgx_datagrid->addColumn(new Structures_DataGrid_Column('Qty (Pcs)', 'good', 'good', array('align' => 'right'), NULL, "cgx_format_3digit()"));
if ($cgx_def_columns['weight']== 1) $cgx_datagrid->addColumn(new Structures_DataGrid_Column('Weight (Kg)', 'weight', 'weight', array('align' => 'right'), NULL, "cgx_format_3digit()"));
