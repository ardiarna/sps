...
$data_ref = npl_fetch_table(
        "SELECT m_inout.*, c_order.document_no so
        FROM m_inout
...
if ($_REQUEST['mode'] != 'edit') {
    ...
    $select_bm = "<img onclick=\"popupReference('pengiriman-barang');\" style='cursor: pointer; vertical-align: bottom;' src='images/icon_reference.png'>";
}
...
echo "<td width='10%'></td>";
echo "<td width='13%'>Tanggal Return {$mandatory}</td>";
...
echo "<td><input readonly='readonly' type='text' id='bk_document_no' size='15' value=\"{$data_ref['document_no']}\">{$select_bm}</td>";
echo "<td></td>";
...
echo "<td><input readonly='readonly' type='text' id='bk_so' size='15' value=\"{$data_ref['so']}\"></td>";
echo "</tr>";
echo "</table>";
echo "</div>";
...
if ($_REQUEST['mode'] == 'edit') {
    ...
    $datagrid->addColumn(new Structures_DataGrid_Column('Jumlah Barang<br>Return', 'qty_koreksi', NULL, array('align' => 'right', 'width' => '10%'), NULL, "cgx_format_3digit"));
    $datagrid->addColumn(new Structures_DataGrid_Column('Sisa', 'balance', NULL, array('align' => 'right', 'width' => '10%'), NULL, "cgx_format_3digit"));
...
if ($_REQUEST['mode'] == 'edit') {
    ...
    echo "<input type='button' value='Simpan Dokumen' onclick=\"xajax_saveRK(xajax.getFormValues('frmRK'));\">";
...
<!--
function setPengirimanBarang(m_inout_id, document_no, m_inout_date, so) {
    ...
    var txt_bk_so = document.getElementById('bk_so');
