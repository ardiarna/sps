...
if (strlen($_REQUEST['pkey']['m_box_inout_id']) > 0) {
    include_once 'trx.xm.edit.php';
} else {
    require_once 'Structures/DataGrid.php';
    require_once 'HTML/Table.php';

        if (is_array($_SESSION[$GLOBALS['APP_ID']]['trx.xm']['columns'])) {
            $cgx_def_columns = $_SESSION[$GLOBALS['APP_ID']]['trx.xm']['columns'];
        } else {
            ...
            $_SESSION[$GLOBALS['APP_ID']]['trx.xm']['columns'] = $cgx_def_columns;
        }

        $cgx_sql = "SELECT * FROM m_box_inout mb, c_bpartner cb WHERE mb.c_bpartner_id=cb.c_bpartner_id AND m_transaction_type_id = 7";
...
//===========================================================================================================================
        ...
        echo "<td></td>\n";
        echo "<td width='20'></td>\n";
        if (has_privilege('trx.box-masuk')) {
    ...
    //    echo "<td width='1' class='datagrid_bar_icon'><a title='Export semua data ke CSV' href='action/trx.xm.php?mode=export-all'><img border='0' src='images/icon_csv.png'></a></td>\n";
    //    echo "<td width='1' class='datagrid_bar_icon'><a title='Kustomisasi kolom' href='javascript:customizeColumn(true);'><img border='0' src='images/icon_columns.png'></a></td>\n";
        ...
        echo "<input type='hidden' name='dg_name' value='trx.xm'>\n";
...
//===============================================================================================================================
    ...
    if ($_SESSION[$GLOBALS['APP_ID']]['trx.xm']['error']) {
        echo "<div class='error'>{$_SESSION[$GLOBALS['APP_ID']]['trx.xm']['error']}</div>";
        unset($_SESSION[$GLOBALS['APP_ID']]['trx.xm']['error']);
    }

    if ($_SESSION[$GLOBALS['APP_ID']]['trx.xm']['info']) {
        echo "<div class='info'>{$_SESSION[$GLOBALS['APP_ID']]['trx.xm']['info']}</div>";
        unset($_SESSION[$GLOBALS['APP_ID']]['trx.xm']['info']);
    }
    ...
    if (has_privilege('trx.box-masuk')) $cgx_datagrid->addColumn(new Structures_DataGrid_Column(NULL, NULL, NULL, array('align' => 'center', 'width' => '1'), NULL, 'cgx_edit()'));
