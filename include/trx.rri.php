...
echo "<div class='title'>Penerimaan Transfer</div>";
...
if (strlen($_REQUEST['pkey']['m_inout_id']) > 0) {
    include_once 'trx.rri.edit.php';
} else {
    require_once 'Structures/DataGrid.php';
    require_once 'HTML/Table.php';

    if (is_array($_SESSION[$GLOBALS['APP_ID']]['trx.rri']['columns'])) {
        $cgx_def_columns = $_SESSION[$GLOBALS['APP_ID']]['trx.rri']['columns'];
    } else {
        ...
        $_SESSION[$GLOBALS['APP_ID']]['trx.rri']['columns'] = $cgx_def_columns;
    }

    $cgx_sql = "SELECT m_inout.*, app_org.organization, ref.document_no document_no_ref FROM m_inout JOIN app_org ON(m_inout.tuj_org_id=app_org.app_org_id) JOIN m_inout ref ON(m_inout.m_inout_id_ref = ref.m_inout_id)";
    $cgx_sql .= " WHERE m_inout.m_transaction_type_id = 3 AND m_inout.tuj_org_id <> 0 AND m_inout.tuj_org_id IS NOT NULL";
    ...
    if ($cgx_def_columns['organization'] == 1) $cgx_datagrid->addColumn(new Structures_DataGrid_Column('Organisasi', 'organization', 'organization', array('align' => 'left'), NULL, NULL));
    if ($cgx_def_columns['document_no_ref'] == 1) $cgx_datagrid->addColumn(new Structures_DataGrid_Column('Dokumen Pengiriman', 'document_no_ref', 'document_no_ref', array('align' => 'left'), NULL, NULL));
    if (has_privilege('trx.rri')) $cgx_datagrid->addColumn(new Structures_DataGrid_Column(NULL, NULL, NULL, array('align' => 'center', 'width' => '1'), NULL, 'cgx_edit()'));
//    if (has_privilege('trx.rri')) $cgx_datagrid->addColumn(new Structures_DataGrid_Column(NULL, NULL, NULL, array('align' => 'center', 'width' => '1'), NULL, 'cgx_delete()'));
