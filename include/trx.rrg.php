...
echo "<div class='title'>".title_header('trx.rrg', org())."</div>";
...
function cgx_delete($data) {
    $href  = "javascript:if(confirm('Hapus baris ini?')){window.location='action/trx.rrg.php";
    $href .= "?backvar=module.php%253F%2526m%253Dtrx.rrg&mode=delete&pkey[m_inout_id]={$data['record']['m_inout_id']}';}";
    $out = "<a href=\"{$href}\"><img title='Delete this row' src='images/icon_delete.png' border='0'></a>";
    return $out;
}

if (strlen($_REQUEST['pkey']['m_inout_id']) > 0) {
    include_once 'trx.rrg.edit.php';
} else {
    require_once 'Structures/DataGrid.php';
    require_once 'HTML/Table.php';

    if (is_array($_SESSION[$GLOBALS['APP_ID']]['trx.rrg']['columns'])) {
        $cgx_def_columns = $_SESSION[$GLOBALS['APP_ID']]['trx.rrg']['columns'];
    } else {
        ...
        $_SESSION[$GLOBALS['APP_ID']]['trx.rrg']['columns'] = $cgx_def_columns;
    }

    $cgx_sql = "SELECT m_inout.*, c_order.remark FROM m_inout left join c_order on m_inout.c_order_id=c_order.c_order_id WHERE m_inout.m_transaction_type_id = 3";
    ...
    echo "</tr></table>\n";
    echo "</form>\n";
    $cgx_sql .= " and ( m_inout.m_inout_id LIKE '%{$cgx_search}%' OR m_inout.document_no LIKE '%{$cgx_search}%' OR remark LIKE '%{$cgx_search}%' OR m_inout.dokumen LIKE '%{$cgx_search}%')";
    ...
    if ($cgx_def_columns['remark'] == 1) $cgx_datagrid->addColumn(new Structures_DataGrid_Column('Sales Order', 'remark', 'remark', array('align' => 'left'), NULL, NULL));
    if ($cgx_def_columns['dokumen'] == 1) $cgx_datagrid->addColumn(new Structures_DataGrid_Column('Keterangan', 'dokumen', 'dokumen', array('align' => 'left'), NULL, NULL));
    if (has_privilege('trx.rrg')) $cgx_datagrid->addColumn(new Structures_DataGrid_Column(NULL, NULL, NULL, array('align' => 'center', 'width' => '1'), NULL, 'cgx_edit()'));
    //if (has_privilege('trx.rrg')) $cgx_datagrid->addColumn(new Structures_DataGrid_Column(NULL, NULL, NULL, array('align' => 'center', 'width' => '1'), NULL, 'cgx_delete()'));
