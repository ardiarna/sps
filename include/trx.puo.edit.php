...
if ($_REQUEST['mode'] == 'edit' && $_SESSION[$APP_ID]['pu_order']['c_order_id'] == $_REQUEST['pkey']['c_order_id']) {
...
if ($_REQUEST['mode'] != 'edit') {
    ...
    $select_partner = "<img onclick=\"popupReference('business-partner-v');\" style='cursor: pointer; vertical-align: bottom;' src='images/icon_reference.png'>";
}
...
echo "<td width='36%'><input{$readonly} name='document_no' type='text' size='15' value=\"{$data['document_no']}\"></td>";
echo "<td width='4%'></td>";
echo "<td width='12%'>No. PPB</td>";
echo "<td width='36%'><input{$readonly} name='reference' type='text' size='30' value=\"{$data['reference_no']}\"></td>";
echo "</tr>";
echo "<tr>";
echo "<td>Vendor {$mandatory}</td>";
...
echo "<td>". cgx_form_select('app_org_id', "SELECT app_org_id, organization FROM app_org", $data['app_org_id'], FALSE, $disabled) ."</td>";
echo "</tr>";
echo "</table>";
echo "</form>";
echo "</div>";
...
if ($_REQUEST['mode'] == 'edit') {
    ...
    echo "<input type='button' value='Simpan Dokumen' onclick=\"xajax_savePUO(xajax.getFormValues('frmPUO'));\">";
    ...
    echo "<input" . ($data['status'] == 'C' ? ' disabled' : '')  . " type='button' value='Close PO' onclick=\"xajax_closePUO('{$data['c_order_id']}');\">";
    echo "</div>";
}
