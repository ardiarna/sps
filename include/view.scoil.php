...
echo "<div class='title'>Stock Coil<div style='font-size: 12px;'>As of " . date($APP_DATETIME_FORMAT) . "</div></div>";
...
if (strlen($_REQUEST['pkey']['m_product_id']) > 0) {
    include_once 'view.scoil.detail.php';
}
else{
    require_once 'Structures/DataGrid.php';
    require_once 'HTML/Table.php';

    if (is_array($_SESSION[$GLOBALS['APP_ID']]['view.scoil']['columns'])) {
        $cgx_def_columns = $_SESSION[$GLOBALS['APP_ID']]['view.scoil']['columns'];
    } else {
        ...
        $_SESSION[$GLOBALS['APP_ID']]['view.scoil']['columns'] = $cgx_def_columns;
    }
    ...
    $cgx_sql = "
        ...
        mso1.m_product_id,
        mso1.*,
        mso1.balance_quantity as balance_qty,
        mp.category,
        mp.product_code, 
        mp.product_name, 
        mp.spec, 
        ...
        FROM 
        m_stock_onhand mso1 INNER JOIN ( SELECT m_product_id, MAX(m_inout_date) m_inout_date FROM m_stock_onhand WHERE app_org_id = '". org() ."' GROUP BY m_product_id ) mso2 ON(mso1.m_product_id = mso2.m_product_id AND mso1.m_inout_date = mso2.m_inout_date)
    ...
    $cgx_sum = "
        SELECT 
        SUM(mso1.balance_quantity) AS balance_qty
    ...
    echo "<td width='1' class='datagrid_bar_icon'><a title='Export semua data ke CSV' href='action/view.scoil.php?mode=export-all'><img border='0' src='images/icon_csv.png'></a></td>\n";
    ...
    echo "<input type='hidden' name='dg_name' value='view.scoil'>\n"; // penyebab ga bisa di pilih
    ...
    echo "<table cellspacing='0' cellpadding='0' class='datagrid_bar_customize' align='left' width='160'><tr><td width='1'><input" . ($cgx_def_columns['balance_qty'] == 1 ? ' checked' : '') . " id='col_balance_qty' name='col[balance_qty]' type='checkbox'></td><td width='99%'><label for='col_balance_quantity'>Balance (Pcs)</label></td></tr></table>\n";
    ...
    echo "<table cellspacing='0' cellpadding='0' class='datagrid_bar_customize' align='left' width='160'><tr><td width='1'><input" . ($cgx_def_columns['category'] == 1 ? ' checked' : '') . " id='col_category' name='col[category]' type='checkbox'></td><td width='99%'><label for='col_decrip'>Category</label></td></tr></table>\n";
    echo "</td>\n";
    ...
    if ($_SESSION[$GLOBALS['APP_ID']]['view.scoil']['error']) {
        echo "<div class='error'>{$_SESSION[$GLOBALS['APP_ID']]['view.scoil']['error']}</div>";
        unset($_SESSION[$GLOBALS['APP_ID']]['view.scoil']['error']);
    }

    if ($_SESSION[$GLOBALS['APP_ID']]['view.scoil']['info']) {
        echo "<div class='info'>{$_SESSION[$GLOBALS['APP_ID']]['view.scoil']['info']}</div>";
        unset($_SESSION[$GLOBALS['APP_ID']]['view.scoil']['info']);
    }
    ...
    if ($cgx_def_columns['balance_qty'] == 1) $cgx_datagrid->addColumn(new Structures_DataGrid_Column('Balance (Pcs)', 'balance_qty', 'balance_qty', array('align' => 'right', 'width' => 100), NULL, NULL));
    if ($cgx_def_columns['weight'] == 1) $cgx_datagrid->addColumn(new Structures_DataGrid_Column('Berat (Kg)', 'weight', 'weight', array('align' => 'right', 'width' => 100), NULL, "cgx_format_money()"));
    ...
    if ($cgx_def_columns['category'] == 1) $cgx_datagrid->addColumn(new Structures_DataGrid_Column('Category', 'category', 'category', array('align' => 'left', 'width' => 100), NULL, "cgx_format_category()"));
    if (has_privilege('view.scoil')) $cgx_datagrid->addColumn(new Structures_DataGrid_Column(NULL, NULL, NULL, array('align' => 'center', 'width' => '1'), NULL, 'cgx_edit()'));
    if (has_privilege('view.scoil')) $cgx_datagrid->addColumn(new Structures_DataGrid_Column(NULL, NULL, NULL, array('align' => 'center', 'width' => '1'), NULL, 'cgx_detail()'));
