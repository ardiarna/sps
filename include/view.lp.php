...
// load default value
$def_org = $_REQUEST['org'] ? $_REQUEST['org'] : 5; //$def_org = 5;  //long pipe (erw)
if ($_REQUEST['d1']) {
    $d1 = $_REQUEST['d1'];
    $d2 = $_REQUEST['d2'];
} else {
...
echo "<div class='title'>Long Pipe Production</div>";
echo "<div class='data_box'>";
echo "<form action='module.php'>";
echo "<input type='hidden' name='m' value='view.lp'>";
echo "<table>";
echo "<tr>";
...
echo "<td colspan='3'>" . cgx_form_select('org', "SELECT app_org_id, organization FROM app_org WHERE is_trx = 'Y' AND (app_org_id = '{$def_org}' OR app_org_id = '6') ORDER BY organization", $def_org, FALSE) . "</td>";
...
$rsx = mysql_query(
        "SELECT m_product_id, product_code, spec, od, thickness, length, SUM(quantity) quantity "
        . "FROM c_production_plan "
