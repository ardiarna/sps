...
function cgx_delete($data) {
    $href  = "javascript:if(confirm('Hapus baris ini?')){window.location='action/trx.rlp.php";
    $href .= "?backvar=module.php%253F%2526m%253Dtrx.rlp&mode=delete&pkey[m_work_order_id]={$data['record']['m_work_order_id']}';}";
...
if (strlen($_REQUEST['pkey']['m_work_order_id']) > 0) {
    include_once 'trx.rlp.edit.php';
} else {
    require_once 'Structures/DataGrid.php';
    require_once 'HTML/Table.php';

    if (is_array($_SESSION[$GLOBALS['APP_ID']]['trx.rlp']['columns'])) {
        $cgx_def_columns = $_SESSION[$GLOBALS['APP_ID']]['trx.rlp']['columns'];
    } else {
        ...
        $_SESSION[$GLOBALS['APP_ID']]['trx.rlp']['columns'] = $cgx_def_columns;
    }
    ...
    $cgx_sql = "SELECT m_work_order.*, COALESCE(remark,c_forecast.document_no) so, COALESCE(c_bpartner.partner_name,cb2.partner_name) partner_name 
        ...
        LEFT JOIN c_bpartner cb2 ON (c_forecast.c_bpartner_id=cb2.c_bpartner_id) WHERE m_work_order.type_id = 'R' ";
    ...
    echo "</tr></table>\n";
    echo "</form>\n";    
    $cgx_sql .= " AND ( m_work_order.m_work_order_id LIKE '%{$cgx_search}%' OR m_work_order.document_no LIKE '%{$cgx_search}%' OR c_order.remark LIKE '%{$cgx_search}%' OR c_bpartner.partner_name LIKE '%{$cgx_search}%' OR cb2.partner_name LIKE '%{$cgx_search}%')";
    ...
    if (has_privilege('trx.rlp')) $cgx_datagrid->addColumn(new Structures_DataGrid_Column(NULL, NULL, NULL, array('align' => 'center', 'width' => '1'), NULL, 'cgx_edit()'));
    if (has_privilege('trx.rlp') AND (user() == 2 OR user() == 46 OR user() == 51)) $cgx_datagrid->addColumn(new Structures_DataGrid_Column(NULL, NULL, NULL, array('align' => 'center', 'width' => '1'), NULL, 'cgx_delete()'));
