...
echo "<div class='title'>Create Work Order Recutting</div>";
echo "<form action='action/trx.ppc-wo-rc.form.php'>";
...
echo "<th class='datagrid_header'>Total Qty</th>";
...
$rsx = mysql_query(
        "SELECT c_production_plan.m_product_id, product_code, spec, od, thickness, length, partner_name, SUM(c_production_plan.quantity) quantity, balance_quantity "
...
//============================================================ 
        . "JOIN m_product ON (c_production_plan.m_product_id = m_product.m_product_id) "
        . "LEFT JOIN (SELECT m_product_id, balance_quantity, max(balance_date) FROM m_stock_balance_d_2 WHERE balance_date <= '{$d1sql}' GROUP BY m_product_id) sb ON (c_production_plan.m_product_id = sb.m_product_id) "
...
echo "<input type='button' value='Back' onclick=\"window.location = 'module.php?m=trx.ppc-wo-rc&yyyy={$def_year}';\">";
echo "</div>";
