...
echo "<div class='title'>Return Barang</div>";
...
function ctl_dok_koreksi($data) {
    $href = "module.php?m=trx.bk&pkey[m_inout_id]={$data['record']['m_inout_id_ref']}";
    return "<a href='{$href}'>{$data['record']['document_no_ref']}</a>";
}
*/

function ctl_dok_koreksi($data) {
    $href = "module.php?m=trx.bk&back_to=trx.rk&mode=view&pkey[m_inout_id]={$data['record']['m_inout_id_ref']}";
    return "<a href='{$href}'>{$data['record']['document_no_ref']}</a>";
}

if (strlen($_REQUEST['pkey']['m_inout_id']) > 0) {
    include_once 'trx.rk.edit.php';
} else {
    ...
    $cgx_sql =
            ...
            "WHERE m_inout.m_transaction_type_id = 5";
    ...
    if ($_SESSION[$GLOBALS['APP_ID']]['trx.rk']['error']) {
        echo "<div class='error'>{$_SESSION[$GLOBALS['APP_ID']]['trx.rk']['error']}</div>";
        unset($_SESSION[$GLOBALS['APP_ID']]['trx.rk']['error']);
    }

    if ($_SESSION[$GLOBALS['APP_ID']]['trx.rk']['info']) {
        echo "<div class='info'>{$_SESSION[$GLOBALS['APP_ID']]['trx.rk']['info']}</div>";
        unset($_SESSION[$GLOBALS['APP_ID']]['trx.rk']['info']);
    }
    ...
    $cgx_datagrid->addColumn(new Structures_DataGrid_Column('Dokumen Di-return', 'document_no_ref', 'document_no_ref', array('align' => 'left'), NULL, "ctl_dok_koreksi()"));
    if (has_privilege('trx.return')) $cgx_datagrid->addColumn(new Structures_DataGrid_Column(NULL, NULL, NULL, array('align' => 'center', 'width' => '1'), NULL, 'cgx_edit()'));
//    if (has_privilege('trx.return')) $cgx_datagrid->addColumn(new Structures_DataGrid_Column(NULL, NULL, NULL, array('align' => 'center', 'width' => '1'), NULL, 'cgx_delete()'));
