...
echo "<div class='title'>".title_header('trx.bki', org())."</div>";
...
if (strlen($_REQUEST['pkey']['m_inout_id']) > 0) {
    include_once 'trx.bki.edit.php';
} else {
    require_once 'Structures/DataGrid.php';
    require_once 'HTML/Table.php';

    $cgx_sql = "SELECT * FROM m_inout JOIN app_org ON(m_inout.tuj_org_id=app_org.app_org_id) WHERE m_inout.m_transaction_type_id = 4 AND m_inout.tuj_org_id <> 0 AND m_inout.tuj_org_id IS NOT NULL ";
    ...
    $cgx_datagrid->addColumn(new Structures_DataGrid_Column('Org Tujuan', 'organization', 'organization', array('align' => 'left'), NULL, NULL));
    $cgx_datagrid->addColumn(new Structures_DataGrid_Column('No. BKBJ', 'dokumen', 'dokumen', array('align' => 'left'), NULL, NULL));
    if (has_privilege('trx.bki')) $cgx_datagrid->addColumn(new Structures_DataGrid_Column(NULL, NULL, NULL, array('align' => 'center', 'width' => '1'), NULL, 'cgx_edit()'));
