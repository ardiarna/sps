...
echo "<div class='title'>".title_header('trx.bkg', org())."</div>";
...
if (strlen($_REQUEST['pkey']['m_inout_id']) > 0) {
    include_once 'trx.bkg.edit.php';
} else {
    require_once 'Structures/DataGrid.php';
    require_once 'HTML/Table.php';

    $cgx_sql = "SELECT m_inout.*, remark, partner_name FROM m_inout 
        LEFT JOIN c_order USING (c_order_id) 
        LEFT JOIN c_bpartner USING (c_bpartner_id) WHERE m_inout.m_transaction_type_id = 4 ";
    ...
    echo "</tr></table>\n";
    echo "</form>\n";
    $cgx_sql .= " and ( m_inout.m_inout_id LIKE '%{$cgx_search}%' OR m_inout.document_no LIKE '%{$cgx_search}%' OR m_inout.dokumen LIKE '%{$cgx_search}%')";
    ...
    $cgx_datagrid->addColumn(new Structures_DataGrid_Column('Remark', 'remark', 'remark', array('align' => 'left'), NULL, NULL));
    $cgx_datagrid->addColumn(new Structures_DataGrid_Column('Pelanggan', 'partner_name', 'partner_name', array('align' => 'left'), NULL, NULL));
    $cgx_datagrid->addColumn(new Structures_DataGrid_Column('No. Kendaraan', 'no_kendaraan', 'no_kendaraan', array('align' => 'left'), NULL, NULL));
    $cgx_datagrid->addColumn(new Structures_DataGrid_Column('Keterangan', 'dokumen', 'dokumen', array('align' => 'left'), NULL, NULL));
    if (has_privilege('trx.bkg')) $cgx_datagrid->addColumn(new Structures_DataGrid_Column(NULL, NULL, NULL, array('align' => 'center', 'width' => '1'), NULL, 'cgx_edit()'));
