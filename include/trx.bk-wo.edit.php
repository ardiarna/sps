...
$data = npl_fetch_table(
        "SELECT m_inout.*, m_work_order.document_no so, partner_name, order_date
        FROM m_inout
...
echo "<td><input readonly='readonly' type='text' id='work_order' size='15' value=\"{$data['so']}\">{$select_wo}</td>";
echo "<td></td>";
...
if ($_REQUEST['mode'] == 'edit') {
    ...
    $datagrid->addColumn(new Structures_DataGrid_Column('Jumah Barang', 'quantity', NULL, array('align' => 'right'), NULL, "cgx_format_3digit"));
