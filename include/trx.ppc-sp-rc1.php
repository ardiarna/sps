...
echo "<div class='title'>SPK - Recutting</div>";
...
function cgx_edit($data) {
    $href = "module.php?&m={$_REQUEST['m']}&pkey[c_spk_id]={$data['record']['c_spk_id']}";
...
if (strlen($_REQUEST['pkey']['c_spk_id']) > 0) {
    include_once 'trx.ppc-sp-rc1.edit.php';
} else {
    require_once 'Structures/DataGrid.php';
    require_once 'HTML/Table.php';

    if (is_array($_SESSION[$GLOBALS['APP_ID']]['trx.ppc-sp-rc1']['columns'])) {
        $cgx_def_columns = $_SESSION[$GLOBALS['APP_ID']]['trx.ppc-sp-rc1']['columns'];
    } else {
        ...
        $_SESSION[$GLOBALS['APP_ID']]['trx.ppc-sp-rc1']['columns'] = $cgx_def_columns;
    }

    $cgx_sql = "SELECT DISTINCT c_spk.*, c_wo.document_no wo_no, wo_week, wo_date, CONCAT(machine_code,' - ',machine_name) machine_code FROM c_spk JOIN c_wo USING (c_wo_id) 
            JOIN m_machine USING (m_machine_id) JOIN c_wo_line USING (c_wo_id) JOIN c_production_plan USING (c_production_plan_id)
            WHERE c_production_plan.app_org_id = 3 AND 1 = 1";
    ...
    echo "<td><input type='button' value='Create SPK' onclick=\"window.location = 'module.php?m=trx.ppc-sp-rc';\"></td>\n";
    echo "<td>\n";
