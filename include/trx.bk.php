...
echo "<div class='title'>".title_header('trx.bk', org())."</div>";
...
function cgx_delete($data) {
    $href  = "javascript:if(confirm('Hapus baris ini?')){window.location='action/trx.bk.php";
    $href .= "?backvar=module.php%253F%2526m%253Dtrx.bk&mode=delete&pkey[m_inout_id]={$data['record']['m_inout_id']}';}";
    $out = "<a href=\"{$href}\"><img title='Delete this row' src='images/icon_delete.png' border='0'></a>";
    return $out;
}

if (strlen($_REQUEST['pkey']['m_inout_id']) > 0) {
    include_once 'trx.bk.edit.php';
} else {
    require_once 'Structures/DataGrid.php';
    require_once 'HTML/Table.php';

    if (is_array($_SESSION[$GLOBALS['APP_ID']]['trx.bk']['columns'])) {
        $cgx_def_columns = $_SESSION[$GLOBALS['APP_ID']]['trx.bk']['columns'];
    } else {
        ...
        $_SESSION[$GLOBALS['APP_ID']]['trx.bk']['columns'] = $cgx_def_columns;
    }

    $cgx_sql = "SELECT m_inout.*, remark, partner_name FROM m_inout 
    LEFT JOIN c_order USING (c_order_id) 
    LEFT JOIN c_bpartner USING (c_bpartner_id) WHERE m_inout.m_transaction_type_id = 4 AND m_inout.c_order_id <> 0 AND m_inout.c_order_id IS NOT NULL";
    ...
    echo "</tr></table>\n";
    echo "</form>\n";
    $cgx_sql .= " and ( m_inout.m_inout_id LIKE '%{$cgx_search}%' OR m_inout.document_no LIKE '%{$cgx_search}%' OR remark LIKE '%{$cgx_search}%' OR partner_name LIKE '%{$cgx_search}%')";
    ...
    if (has_privilege('trx.barang-keluar')) $cgx_datagrid->addColumn(new Structures_DataGrid_Column(NULL, NULL, NULL, array('align' => 'center', 'width' => '1'), NULL, 'cgx_edit()'));
