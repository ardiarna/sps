...
echo "<div class='title'>Bukti Keluar Plat Coil - WO Slit</div>";
...
function cgx_delete($data) {
    $href  = "javascript:if(confirm('Hapus baris ini?')){window.location='action/trx.bkpc-wo.php";
    $href .= "?backvar=module.php%253F%2526m%253Dtrx.bkpc-wo&mode=delete&pkey[m_bkpc_id]={$data['record']['m_bkpc_id']}';}";
...
if (strlen($_REQUEST['pkey']['m_bkpc_id']) > 0) {
    include_once 'trx.bkpc-wo.edit.php';
} else {
    require_once 'Structures/DataGrid.php';
    require_once 'HTML/Table.php';

    if (is_array($_SESSION[$GLOBALS['APP_ID']]['trx.bkpc-wo']['columns'])) {
        $cgx_def_columns = $_SESSION[$GLOBALS['APP_ID']]['trx.bkpc-wo']['columns'];
    } else {
        ...
        $_SESSION[$GLOBALS['APP_ID']]['trx.bkpc-wo']['columns'] = $cgx_def_columns;
    }
    ...
    if ($_SESSION[$GLOBALS['APP_ID']]['trx.bkpc-wo']['error']) {
        echo "<div class='error'>{$_SESSION[$GLOBALS['APP_ID']]['trx.bkpc-wo']['error']}</div>";
        unset($_SESSION[$GLOBALS['APP_ID']]['trx.bkpc-wo']['error']);
    }

    if ($_SESSION[$GLOBALS['APP_ID']]['trx.bkpc-wo']['info']) {
        echo "<div class='info'>{$_SESSION[$GLOBALS['APP_ID']]['trx.bkpc-wo']['info']}</div>";
        unset($_SESSION[$GLOBALS['APP_ID']]['trx.bkpc-wo']['info']);
    }
    ...
    if ($cgx_def_columns['sj_no'] == 1) $cgx_datagrid->addColumn(new Structures_DataGrid_Column('Nomor WO', 'sj_no', 'sj_no', array('align' => 'left'), NULL, NULL));
    ...
    if (has_privilege('trx.bkpc-wo')) $cgx_datagrid->addColumn(new Structures_DataGrid_Column(NULL, NULL, NULL, array('align' => 'center', 'width' => '1'), NULL, 'cgx_edit()'));
    //if (has_privilege('trx.bkpc-wo')) $cgx_datagrid->addColumn(new Structures_DataGrid_Column(NULL, NULL, NULL, array('align' => 'center', 'width' => '1'), NULL, 'cgx_delete()'));
