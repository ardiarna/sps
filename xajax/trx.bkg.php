...
function saveBK($data) {
    ...
    if ($data['m_inout_id']) {
        $sql = "UPDATE m_inout SET m_inout_date = '" . cgx_dmy2ymd($data['m_inout_date']) . "', c_order_id = '{$data['c_order_id']}', no_kendaraan = '{$data['no_kendaraan']}', dokumen = '{$data['dokumen']}' WHERE m_inout_id = '{$data['m_inout_id']}'";
    } else {
        require_once 'lib/class.Penomoran.php';
        $nomor = new Penomoran();
        $document_no = $nomor->urut('BK', org());
        $sql =
        "INSERT INTO m_inout (app_org_id, document_no, m_inout_date, m_transaction_type_id, c_order_id, no_kendaraan, dokumen)
         VALUES ('" . org() . "',  '{$document_no}', '" . cgx_dmy2ymd($data['m_inout_date']) . "', 4, '{$data['c_order_id']}','{$data['no_kendaraan']}','{$data['dokumen']}')";
    }
    ...
    $_SESSION[$APP_ID]['trx.bkg']['info'] = "Dokumen sudah berhasil disimpan";
    $res->script("window.location = 'module.php?m=trx.bkg&pkey[m_inout_id]={$bki_id}';");
    return $res;
}
