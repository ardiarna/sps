...
function editForm($line_no) {
    ...
    foreach ($_SESSION[$APP_ID]['order']['lines'] as $line) {
...
function saveSO($data) {
    ...
    if ($data['c_order_id']) {
        $sql = "UPDATE c_order SET app_org_id = '{$data['app_org_id']}', document_no = '{$data['document_no']}', 
            c_bpartner_id = '{$data['c_bpartner_id']}', order_date = '" . cgx_dmy2ymd($data['order_date']) . "', 
            reference_no = '{$data['reference']}', remark = '{$data['remark']}', update_date = NOW(), update_user = '" . user() . "'
    ...
    $_SESSION[$APP_ID]['trx.so']['info'] = "Dokumen sudah berhasil disimpan";
    
    $res->script("window.location = 'module.php?m=trx.so&pkey[c_order_id]={$order_id}';");
    return $res;
}
