...
function editForm($line_no) {
    ...
    foreach ($_SESSION[$APP_ID]['rr']['lines'] as $line) {
    ...
    $html .= "<td width='33%'><input type='text' size='25' name='no_coil' value=\"{$data['no_coil']}\" style='text-align: left;'></td>";
    $html .= "</tr>";
    $html .= "<tr>";
    $html .= "<td>Barang</td>";
    ...
    $html .= "<td><input type='text' size='25' name='no_lot' value=\"{$data['no_lot']}\" style='text-align: left;'></td>";
    $html .= "</tr>";
    $html .= "<tr>";
    ...
    $html .= "<td></td>";
    $html .= "<td>No Box</td>";
    $html .= "<td><input type='text' size='25' name='no_box' value=\"{$data['no_box']}\" style='text-align: left;'></td>";
    $html .= "</tr>";
    $html .= "<tr>";
...
function deleteLine($line_no) {
    ...
    $rr = $_SESSION[$APP_ID]['rr'];
    foreach ($rr['lines'] as $k => $line) {
        if ($line['line'] == $line_no) {
            $del = $k;
            if ($line['m_inout_line_id']) {
                if ($rr['delete']) {
                    $rr['delete'][] = $line['m_inout_line_id'];
                } else {
                    $rr['delete'] = array($line['m_inout_line_id']);
                }
            }
        }
    }
    unset($rr['lines'][$del]);
    $_SESSION[$APP_ID]['rr'] = $rr;

    $res = new xajaxResponse();
    $res->script("xajax_showLines('{$rr['m_inout_id']}', 'edit');");
...
function saveRR($data) {
    ...
    if ($data['m_inout_id']) {
        $sql = "UPDATE m_inout SET m_inout_date = '" . cgx_dmy2ymd($data['m_inout_date']) . "', dokumen = '{$data['dokumen']}', 
            tuj_org_id = '{$data['c_bpartner_id']}', update_date = NOW(), update_user = '" . user() . "' WHERE m_inout_id = '{$data['m_inout_id']}'";
    } else {
        require_once 'lib/class.Penomoran.php';
        $nomor = new Penomoran();
        $document_no = $nomor->urut('BM', org());
        $sql =
            "INSERT INTO m_inout (app_org_id, document_no, tuj_org_id, m_inout_date, m_transaction_type_id, dokumen, create_user)
            VALUES ('" . org() . "',  '{$document_no}', '{$data['c_bpartner_id']}', '" . cgx_dmy2ymd($data['m_inout_date']) . "', 3, '{$data['dokumen']}', '". user() ."')";
    }    
    ...
    if ($data['m_inout_id']) {
        $rr_id = $data['m_inout_id'];
    } else {
        $rr_id = mysql_insert_id($APP_CONNECTION);
    }
    foreach ($rr['lines'] as $line) {
        if ($line['m_inout_line_id']) {  // JIKA BMBJ EDIT
            ...
            $sql = "UPDATE m_inout_line SET m_product_id = '{$line['m_product_id']}', m_warehouse_id = '{$line['m_warehouse_id']}', no_box = '{$line['no_box']}', no_coil = '{$line['no_coil']}', 
                    no_lot = '{$line['no_lot']}', quantity = '{$line['quantity']}' WHERE m_inout_line_id = '{$line['m_inout_line_id']}'";
            $rsx = mysql_query($sql, $APP_CONNECTION);
            ...
            $sql = "INSERT INTO m_inout_line (m_inout_id, m_product_id, quantity, m_warehouse_id, m_work_order_line_id, no_box, no_coil, no_lot)
                    VALUES ('{$rr_id}', '{$line['m_product_id']}', '{$line['quantity']}', '{$line['m_warehouse_id']}', '{$line['m_work_order_line_id']}', '{$line['no_box']}', '{$line['no_coil']}', '{$line['no_lot']}')";
            $rsx = mysql_query($sql, $APP_CONNECTION);
            //=================================================================================================== update balance
    ...
    $_SESSION[$APP_ID]['trx.rr']['info'] = "Dokumen sudah berhasil disimpan";
    $res->script("window.location = 'module.php?m=trx.rr&pkey[m_inout_id]={$rr_id}';");
    return $res;
}
...
function updateLine($data) {
    ...
    if (is_array($rr['lines'])) {
        foreach ($rr['lines'] as $k => $d) {
            if ($d['line'] == $data['line']) {
                $rr['lines'][$k]['quantity'] = $data['quantity'];
                $rr['lines'][$k]['m_product_id'] = $data['m_product_id'];
                $rr['lines'][$k]['m_warehouse_id'] = $data['m_warehouse_id'];
                //$rr['lines'][$k]['m_work_order_line_id'] = $data['m_work_order_line_id'];
                $rr['lines'][$k]['work_order'] = $data['work_order'];
                $rr['lines'][$k]['no_box'] = $data['no_box'];
                $rr['lines'][$k]['no_coil'] = $data['no_coil'];
                $rr['lines'][$k]['no_lot'] = $data['no_lot'];
                $product = cgx_fetch_table("SELECT * FROM m_product WHERE m_product_id = '{$data['m_product_id']}'");
                if (is_array($product)) foreach ($product as $pk => $pv) if (!is_numeric($pk)) $rr['lines'][$k][$pk] = $pv;
                $warehouse = cgx_fetch_table("SELECT * FROM m_warehouse WHERE m_warehouse_id = '{$data['m_warehouse_id']}'");
                if (is_array($warehouse)) foreach ($warehouse as $pk => $pv) if (!is_numeric($pk)) $rr['lines'][$k][$pk] = $pv;
                $line_updated = TRUE;
                break;
            }
        }
