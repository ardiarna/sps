...
function saveXM($data) {
    ...
    $nomor = new Penomoran();
    $document_no = $nomor->urut('XM', org());
    $sql =
        "INSERT INTO m_box_inout (app_org_id, document_no, m_box_inout_date, m_transaction_type_id, c_bpartner_id, no_truck, nama_supir)
        VALUES ('" . org() . "', '{$document_no}', '" . cgx_dmy2ymd($data['m_box_inout_date']) . "', 7, '".mysql_escape_string($data['c_bpartner_id'])."','".mysql_escape_string($data['no_truck'])."','".mysql_escape_string($data['nama_supir'])."')";
    ...
    $xm_id = mysql_insert_id($APP_CONNECTION);
    
    foreach ($xm['lines'] as $line) {
        $sql = "INSERT INTO m_box_inout_line (m_box_inout_id, m_box_id)
            VALUES ('{$xm_id}', '{$line['m_box_id']}')";
        $rsx = mysql_query($sql, $APP_CONNECTION);
        $sql = "UPDATE m_box SET location = 'I' WHERE m_box_id = '{$line['m_box_id']}'";
        $rsx = mysql_query($sql, $APP_CONNECTION);
    }

    $_SESSION[$APP_ID]['trx.xm']['info'] = "Dokumen sudah berhasil disimpan";
    $res->script("window.location = 'module.php?m=trx.xm&pkey[m_box_inout_id]={$xm_id}';");
    return $res;
}
...
$xajax->register(XAJAX_FUNCTION, 'saveXM');
