...
function editForm($line_no) {
    ...
    foreach ($_SESSION[$APP_ID]['bkpc-wo']['lines'] as $line) {
...
function saveBKPC($data) {
    ...
    if ($data['m_bkpc_id']) {
        $sql = "UPDATE m_bkpc SET bkpc_date = '" . cgx_dmy2ymd($data['bkpc_date']) . "', document_no = '{$data['document_no']}',
          sj_no = '{$data['sj_no']}', update_date = NOW(), 
          update_user = '" . user() . "' WHERE m_bkpc_id = '{$data['m_bkpc_id']}'";
    } else {
        $sql = "INSERT INTO m_bkpc (app_org_id, document_no, partner_name , sj_no, bkpc_date, bkpc_type, create_user, create_date )
        VALUES ('". org() . "', '{$data['document_no']}', '{$data['partner_name']}', '{$data['sj_no']}', 
        '" . cgx_dmy2ymd($data['bkpc_date']) . "', '1', '". user()  ."', NOW())";
    }
    ...
    $_SESSION[$APP_ID]['trx.bkpc-wo']['info'] = "Dokumen sudah berhasil disimpan";
    $res->script("window.location = 'module.php?m=trx.bkpc-wo&pkey[m_bkpc_id]={$bkpc_id}';");
    return $res;
}
