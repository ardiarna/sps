...
//    }
    ...
    foreach ($data['lines'] as $m_work_order_line_id => $quantity) {
        $line = cgx_fetch_table(
            ...
            "FROM m_work_order_line " .
            "JOIN m_product ON (m_work_order_line.m_product_id = m_product.m_product_id) " .
            "LEFT JOIN m_stock_balance ON (m_work_order_line.m_product_id = m_stock_balance.m_product_id AND latest = 'Y') " .
            "WHERE m_work_order_line_id = '{$m_work_order_line_id}'");
        ...
        mysql_query(
                "UPDATE m_work_order_line SET delivered_quantity = delivered_quantity + {$quantity0} " .
                "WHERE m_work_order_line_id = '{$m_work_order_line_id}'",
                $APP_CONNECTION);
...
function grid_qty_form($data) {
    ...
    $def_delivery = $data['record']['order_quantity'] - $data['record']['delivered_quantity'];
