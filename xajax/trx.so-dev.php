...
function saveSO($data) {
    ...
    if ($data['c_order_id']) {
        $sql = "UPDATE c_order SET c_bpartner_id = '{$data['c_bpartner_id']}',
        ...
        $sql =
            ...
            VALUES ('" . org() . "', '{$document_no}', '{$data['c_bpartner_id']}', '" . cgx_dmy2ymd($data['order_date']) . "',
    ...
    $_SESSION[$APP_ID]['trx.so-dev']['info'] = "Dokumen sudah berhasil disimpan";
    
    $res->script("window.location = 'module.php?m=trx.so-dev&pkey[c_order_id]={$order_id}';");
    return $res;
}
...
function closeSO($c_order_id, $confirmed = FALSE) {
    ...
    if ($confirmed) {
        ...
        $res->script("window.location = 'module.php?m=trx.so-dev&pkey[c_order_id]={$c_order_id}';");
        return $res;
    } else {
        $outstanding = FALSE;
        $order = $_SESSION[$APP_ID]['order'];
